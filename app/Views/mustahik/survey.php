                    ...
                    <div class="card border-0 shadow-sm mb-3 rounded-3"
                        ...
                        <div class="card-body">
                            ...
                            <div ng-if="q.type == 'radio'" class="ps-2">
                                <div class="custom-control custom-radio form-check mb-2"
                                    ng-repeat="ops in q.opsi track by $index">
                                    <input type="radio"
                                        id="customRadio{{q.id}}_{{$index}}"
                                        name="radio_{{q.id}}"
                                        class="custom-control-input"
                                        value="{{ops.label}}"
                                        ng-checked="q.jawaban === ops.label"
                                        ng-click="q.jawaban = ops.label"
                                        ng-disabled="datas.survey.length !=0">
                                    <label for="customRadio{{q.id}}_{{$index}}" class="custom-control-label">
                                        {{ops.label}}
                                    </label>
                                </div>
                            </div>
                            ...
                            <div ng-if="q.type == 'number' && q.id != '29'">
                                <input type="text"
                                    ...
                                    mask-currency="'Rp. '" config="{group:'.',decimalSize:'0',indentation:' '}">
                            </div>
                            <div ng-if="q.type == 'number' && q.id == '29'">
                                <input type="text"
                                    class="form-control form-control-lg border-primary shadow-sm rounded-2"
